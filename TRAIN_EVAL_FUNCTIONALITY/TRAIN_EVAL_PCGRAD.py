"""Module consolidating entire pipeline to train SuTraN using PCGrad by 
Yu et al. [1]_, and subsequently evaluate it on a test set. 
...
def train_eval(log_name, 
    ...
    PCGrad MTO technique by Yu et al. [1]_. 
    ...
    # Optimization (MTO) technique utilized being PCGrad 
    backup_path = os.path.join(intermediate_path, "PCGrad")
    ...
    # Specify modulelist containing all SuTraN layers containing 
    # shared parameters 
    shared_layers = nn.ModuleList()

    # Loop through all submodules of the SuTraN model
    for name, module in model.named_children():
        # Check if the module is NOT task-specific
        if name not in ['fc_out_act', 'fc_out_ttne', 'fc_out_rrt', 'fc_out_out']:
            # Add to shared_layers
            shared_layers.append(module)

    
    # Training procedure 
    from SuTraN.PCGrad_train_procedure import train_model
    start_epoch = 0
    num_epochs = 200 
