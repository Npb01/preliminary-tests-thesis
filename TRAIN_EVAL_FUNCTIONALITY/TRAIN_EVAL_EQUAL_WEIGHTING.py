...
def subset_split_by_case(
    ...
    """
    Select a case-level subset of a split and filter all tensors by it.
    """
    if subset_fraction >= 1.0:
        unique_case_ids = torch.unique(case_ids)
        keep_mask = torch.ones_like(case_ids, dtype=torch.bool)
        return data_tuple, case_ids, keep_mask, unique_case_ids

    unique_case_ids = torch.unique(case_ids).cpu().numpy()
    num_cases_total = len(unique_case_ids)
    num_cases_keep = max(1, int(round(num_cases_total * subset_fraction)))

    rng = np.random.default_rng(seed)
    chosen_case_ids = rng.choice(unique_case_ids, size=num_cases_keep, replace=False)

    chosen_case_ids_tensor = torch.as_tensor(
        chosen_case_ids,
        dtype=case_ids.dtype,
        device=case_ids.device,
    )
    keep_mask = torch.isin(case_ids, chosen_case_ids_tensor)

    filtered_data_tuple = tuple(tensor[keep_mask] for tensor in data_tuple)
    filtered_case_ids = case_ids[keep_mask]

    return filtered_data_tuple, filtered_case_ids, keep_mask, chosen_case_ids_tensor
...
def train_eval(log_name, 
    ...
    if subset_fraction <= 0.0 or subset_fraction > 1.0:
        raise ValueError(
            "The `subset_fraction` parameter should be a float in the "
            "range (0.0, 1.0]."
        )

    if val_subset_fraction is None:
        val_subset_fraction = 1.0

    if val_subset_fraction <= 0.0 or val_subset_fraction > 1.0:
        raise ValueError(
            "The `val_subset_fraction` parameter should be a float in the "
            "range (0.0, 1.0]."
        )
    ...
    # NEW: apply case-level subsetting
    ...
    # NEW: partial validation subsetting (per-epoch validation only, keeps
    # enough batches to avoid noisy checkpoint selection). test_dataset is
    # deliberately left untouched so final test-set metrics stay comparable
    # across configs regardless of val_subset_fraction.
    val_dataset, og_caseint_val, val_keep_mask, _ = subset_split_by_case(
        val_dataset, og_caseint_val, val_subset_fraction, seed + 2,
    )
    # test_dataset, og_caseint_test, test_keep_mask, _ = subset_split_by_case(
    #     test_dataset, og_caseint_test, subset_fraction, seed + 3,
    # )
    ...
    if balance_losses:
        ...
        # losses vs upweighting rrt ONLY), and without this they collide on the
        # same folder name and silently overwrite each other.
    ...
    train_model(model, 
                ...
                out_type=out_type, 
                num_outclasses=num_outclasses,
                patience=patience, # originally 24
...
if __name__ == "__main__":
    ...
    parser.add_argument("--subset_fraction", type=float, required=False, default=1.0,
    ...
    parser.add_argument("--scale_ttne", type=float, required=False, default=1.0,
                        help="Static rescaling factor for the ttne loss (only used if --balance_losses).")
    parser.add_argument("--scale_rrt", type=float, required=False, default=1.0,
                        help="Static rescaling factor for the rrt loss (only used if --balance_losses).")
    parser.add_argument("--val_subset_fraction", type=float, required=False, default=1.0,
    ...
    parser.add_argument("--patience", type=int, required=False, default=24,
