"""
Core SuTraN+ sequence-to-sequence model, built on the original SuTraN
architecture of Wuyts et al. [1]_.

Defines the shared encoder-decoder Transformer that powers every SuTraN
training procedure, including optional remaining-runtime and outcome
heads. The module exposes the `SuTraN` class together with the shared
`PositionalEncoding` block used by both prefix encoders and suffix
decoders.

References
----------
.. [1] Wuyts, B., Vanden Broucke, S., & De Weerdt, J. (2024, October).
       Sutran: an encoder-decoder transformer for full-context-aware
       suffix prediction of business processes. In 2024 6th International
       Conference on Process Mining (ICPM) (pp. 17-24). IEEE.
"""
...
class SuTraN(nn.Module):
    def __init__(self, 
        ...
        and decoder. Build on top of the original SuTraN implementation by
        Wuyts et al. [1]_.
        ...
        outcome_bool : bool, optional 
            ...
            head for outcome prediction. By default `False`. If 
        ...
        References
        ----------
        .. [1] Wuyts, B., Vanden Broucke, S., & De Weerdt, J. (2024, 
               October). Sutran: an encoder-decoder transformer for 
               full-context-aware suffix prediction of business 
               processes. In 2024 6th International Conference on Process 
               Mining (ICPM) (pp. 17-24). IEEE.
        """
        ...
        self.bin_outbool = (self.out_type=='binary_outcome')
        # multiclass out bool
        self.multic_outbool = (self.out_type=='multiclass_outcome')
        ...
        if self.outcome_bool:
            if self.bin_outbool:
                ...
                self.fc_out_out = nn.Linear(self.d_model, self.num_outclasses)
    ...
    def forward(self, 
        ...
        -------
        tuple
            During training (`self.training=True`), returns the tuple of
            tensors required by the active loss builder:
            `(act_logits, ttne_pred [, rrt_pred] [, outcome_pred])`
            where `act_logits` has shape `(B, W, num_activities)`,
            `ttne_pred` is `(B, W, 1)`, `rrt_pred` is present when
            `remaining_runtime_head=True`, and `outcome_pred` is either
            `(B, W, 1)` (binary) or `(B, W, num_outclasses)` before the
            first-step slice.
        tuple
            During inference (`self.training=False`), returns decoded
            suffixes rather than logits:
            `(suffix_acts, suffix_ttne, [rrt_pred], [outcome_pred])`
            where `suffix_acts` and `suffix_ttne` hold greedy decoded
            activity/timestamp sequences of shape `(B, W)`, `rrt_pred`
            is `(B,)` when enabled, and `outcome_pred` is `(B,)` for
            binary or `(B, num_outclasses)` for multiclass outcomes.
        """
        ...
        # auxiliary index for understandability
        ...
        if self.training: # Teacher forcing (for now)
            ...
            if self.only_rrt:
                ...
                # Outcome predictions. Shape 
                #   - (batch_size, window_size, 1) in case of BO
                #   - (batch_size, window_size, num_outclasses) in case of MCO
                out_pred = self.fc_out_out(dec_output) 
                if self.bin_outbool:
                    out_pred = self.sigmoid_out(out_pred) # (batch_size, window_size, 1)

                # Only first decoding step output needed. Shape 
                #   - (batch_size, 1) in case of BO
                #   - (batch_size, num_outclasses) in case of MCO
                out_pred = out_pred[:, 0, :] 
                ...
                if dec_step == 0:
                    ...
                    if self.outcome_bool:
                        ...
                        if self.bin_outbool:
                            ...
                            # Slicing out first decoding step prediction only
                            # in shape required by inference metrics 
                            out_pred = out_pred[:, 0, 0] # (batch_size, )
                        elif self.multic_outbool: 
                            # Slicing out first decoding step prediction only 
                            out_pred = out_pred[:, 0, :] # shape (batch_size, num_outclasses)

                # Decoding activity preditions (greedily)
                #   "Masking padding token"
                act_outputs[:, 0] = -1e9 # (B, C)
            ...
            if self.only_rrt:
                ...
                # (B, W), (B, W) and {(B,) or (B, num_outclasses)}
            elif self.both:
                return suffix_acts_decoded, suffix_ttne_preds, rrt_pred, out_pred
                # (B, W), (B, W), (B,) and {(B,) or (B, num_outclasses)}
            else:
