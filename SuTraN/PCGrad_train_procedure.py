"""
Train loop tailored to PCGrad-based multi-task optimization.

This module mirrors the baseline SuTraN training workflow but injects
PCGrad (Yu et al., 2020) to project conflicting task gradients before
updating the shared parameters. It tracks per-task losses, pairwise
cosine similarities, conflict frequencies, and average batch weights, and
exposes `train_epoch`/`train_model` helpers that wrap the SuTraN model in
a PCGrad-aware update cycle (activity suffix, timestamp suffix,
optional remaining runtime, optional outcome).

References
----------
Yu, T., Kumar, S., Gupta, A., Levine, S., Hausman, K., & Finn, C. (2020). 
Gradient surgery for multi-task learning. Advances in neural information 
processing systems, 33, 5824-5836.
"""
...
from SuTraN.train_utils_PCGrad import MultiOutputLoss_PCGrad
...
def create_task_string_matrix(component_string_list):
    """Create auxiliary numpy array and torch 
    indexing tensors for recording, retrieving and 
    reporting on PCGrad statistics for unique 
    task-pairs. 
    
    np.ndarray of the task string matrix 
    for mapping the unique task pairs to the 
    correct strings for the unique task-pairs for which 
    PCGrad metrics are recorded. 

    Parameters
    ----------
    component_string_list : list of str 
        List containing the strings of the num_tasks prediction targets 
        involved in the experimental setup, in the same order of the 
        model prediction outputs. 
    
    Returns
    -------
    # np_task_string_matrix : np.ndarray 
    #     np.ndarray of the task string matrix 
    #     for mapping the unique task pairs to the 
    #     correct strings for the unique task-pairs for which 
    #     PCGrad metrics are recorded. Shape (num_tasks, num_tasks). 
    unique_pairnames : np.ndarray 
        Numpy arra of dtype U33 (string) and shape `(num_unique_pairs,)`, 
        containing the unique pairnames (of tasks) in the appropriate 
        order. Needed for printing and storing average PCGrad statistics 
        regarding the relations between the gradients of the shared 
        parameters with respect to the different individual task losses. 
    row_indices : torch.Tensor 
        Tensor of shape `(num_unique_pairs,)`, containing the row indices 
        of the upper traingular part of the row by col matrix with offset 
        1. Needed for selecting the upper traingular part, pertaining 
        to the metrics of the unique task-pairs. Stored on 'cuda' (gpu) 
        device if available, cpu otherwise. Dtype torch.int64. 
    col_indices : torch.Tensor 
        Tensor of shape `(num_unique_pairs,)`, containing the col indices 
        of the upper traingular part of the row by col matrix with offset 
        1. Needed for selecting the upper traingular part, pertaining 
        to the metrics of the unique task-pairs. Stored on 'cuda' (gpu) 
        device if available, cpu otherwise. Dtype torch.int64. 
    ...
    """
    num_tasks = len(component_string_list)
    task_string_matrix = []
    for rowstring in component_string_list: 
        row_list = []
        for colstring in component_string_list:
            pair_string = rowstring + '_' + colstring
            row_list.append(pair_string)
        task_string_matrix.append(row_list)
    
    np_task_string_matrix = np.array(task_string_matrix)

    row_indices, col_indices = torch.triu_indices(num_tasks, num_tasks, offset=1, device=device) # shape (num_tasks, num_tasks)

    # create numpy version of row_indices and col_indices 
    np_row_indices = row_indices.cpu().numpy() # shape (num_unique_pairs, )
    np_col_indices = col_indices.cpu().numpy() # shape (num_unique_pairs, )

    # Derive rank 1 array of unique pairnames in the correct order 
    unique_pairnames = np_task_string_matrix[np_row_indices, np_col_indices] # shape (num_unique_pairs,)




    # return np_task_string_matrix, row_indices, col_indices
    return unique_pairnames, row_indices, col_indices
...
def log_variables(writer, 
    ...
    component_string_list : str
        ...
        # cross-entropy). 
    scalar_value_list : list of float or np.array of float 
...
def train_epoch(model, 
                ...
                writer_bool):
    """
    Run one PCGrad training epoch.

    Iterates over the training loader, computes task-specific gradients,
    applies PCGrad projections to resolve conflicts, updates the shared
    optimizer, and accumulates both loss curves and PCGrad diagnostics
    (batch weights, original gradient norms, cosine similarities, and
    conflict frequencies). Supports optional remaining-runtime and
    outcome heads, outcome masking, and TensorBoard logging.

    Parameters
    ----------
    model : SuTraN
        Model in training mode; forward must emit the task outputs
        expected by ``loss_fn``.
    ...
    optimizer : torch.optim.Optimizer
        Optimizer whose step is taken after PCGrad projects the task
        gradients.
    loss_fn : Callable
        PCGrad loss wrapper returning ``(batch_weight, loss_items,
        cosine_similarity_matrix, float_conflict_matrix, og_norm)`` for
        each batch; see ``MultiOutputLoss_PCGrad`` for details.
    batch_interval : int
    ...
    writer : torch.utils.tensorboard.SummaryWriter
        TensorBoard writer for logging losses and PCGrad statistics.
    writer_bool : bool
    ...
    optimizer : torch.optim.Optimizer
        Optimizer with updated momenta/state.
    epoch_averages : tuple of float
        Aggregated losses for each active task (activity, TTNE, optional
        remaining runtime, optional outcome) used for downstream tracking.
    """

    # Track per-head losses for reporting/averaging at epoch end
    ...
    # Create numpy task string matrix for Mapping indices Statistics PCGrad task pairs
    unique_pairnames, row_indices, col_indices = create_task_string_matrix(component_string_list) # np.ndarray of shape (num_tasks, num_tasks)

    # Global trackers for PCGrad statistics across the epoch
    batch_weight_global = np.zeros((num_target_tens,), dtype=np.float64) # shape (num_tasks, )

    # original gradients 
    og_norm_global = np.zeros((num_target_tens,), dtype=np.float64) # shape (num_tasks, )

    # incremental tracking various quantities / statistics PCGrad 
    running_cosine_similarity_matrix = torch.zeros(size=(num_target_tens, num_target_tens), dtype=torch.float32, device=device) 
    running_float_conflict_matrix = torch.zeros(size=(num_target_tens, num_target_tens), dtype=torch.float32, device=device)
    ...
    for batch_num, data in tqdm(enumerate(training_loader), desc="Batch calculation at epoch {}.".format(epoch_number)):
        ...
        # incrementing auxiliary counter if valid batch 
        ...
        # Backprop gradients with PCGrad and return PCGrad metrics 
        batch_weight, loss_items, cosine_similarity_matrix, float_conflict_matrix, og_norm = loss_fn(outputs, labels, instance_mask_out)

        # Perform gradient update based on the gradients computed by PCGrad. 
        optimizer.step()

        # Keeping track of individual loss values 
        ...
        # Track PCGrad metrics incrementally so we can average them later
        # Incrementing running sum of batch weights (to compute average at the end)
        batch_weight_global += batch_weight # np array of shape (num_tasks, )

        # Incrementing running sum of original gradient norms 
        og_norm_global += og_norm # np array of shape (num_tasks,)


        # Incrementing 
        running_cosine_similarity_matrix += cosine_similarity_matrix # shape (num_tasks, num_tasks)
        running_float_conflict_matrix += float_conflict_matrix # shape (num_tasks, num_tasks)
        ...
        if batch_num % batch_interval == (batch_interval-1):
                ...
                # Compute running statistics for unique task pairs' gradients PCGrad 
                running_pairwise_cosine_similarities = running_cosine_similarity_matrix[row_indices, col_indices] # shape (num_unique_pairs,)
                running_average_cosine_sims = running_pairwise_cosine_similarities / num_batches_processed # shape (num_unique_pairs,)

                running_pairwise_conflicts = running_float_conflict_matrix[row_indices, col_indices] # shape (num_unique_pairs,)
                running_percentage_conflicting = running_pairwise_conflicts / num_batches_processed # shape (num_unique_pairs,)
                print("_______________________")
                print("PCGrad Statistics Task Pairs' OG Gradients:")
                print("-----------------")
                for i, pairname in enumerate(unique_pairnames):
                    avg_cosim = running_average_cosine_sims[i].cpu().item()
                    perc_conf = running_percentage_conflicting[i].cpu().item()
                    print("Average Cosine Similarity {}: {}".format(pairname, avg_cosim))
                    print("Fraction of Iterations Conflicting Gradients {}: {}".format(pairname, perc_conf))

                # Compute average batch weight each task gradient 
                # (measure for average magnitude conflict(s) with other task gradients)
                average_batch_weight = batch_weight_global / num_batches_processed # np array of shape (num_tasks,)

                # Compute average gradient norm each task's (shared) gradient (prior to projection)
                running_average_ognorm = og_norm_global / num_batches_processed # np.array shape (num_tasks,)


                # Compute number of other tasks each task's gradient conflicted with in each iteration
                running_num_conflicts_task = torch.sum(running_float_conflict_matrix, dim=-1) # shape (num_tasks,)
                # Compute average fraction of other tasks' gradients each task's grad conflicted 
                # with over all batches 
                running_task_conflict_fraction = (running_num_conflicts_task / num_batches_processed) / (num_target_tens-1) # shape (num_tasks,)

                print("_______________________")
                print("PCGrad Statistics Individual Tasks' OG Gradients:")
                print("-----------------")


                for i, task_name in enumerate(component_string_list):
                    print("----")
                    print(task_name)
                    print("----")
                    avg_task_batchweight = average_batch_weight[i]
                    avg_task_ognorm = running_average_ognorm[i]
                    avg_task_conflict_percentage= running_task_conflict_fraction[i].cpu().item() * 100 
                    print("{} gradient - Average percentage of other tasks' gradients conflicting: {}".format(task_name, avg_task_conflict_percentage))
                    print("{} gradient - Average batch weight: {}".format(task_name, avg_task_batchweight))
                    print("{} gradient - Average OG L2 Gradient Norm (magnitude): {}".format(task_name, avg_task_ognorm))



    # ========================================================
    # ==  Computing average los metrics and (PCGrad) stats  ==
    # ========================================================
    ...
    print("_______________________")
    print("PCGrad Statistics Indidual Tasks' OG Gradients:")
    print("-----------------")
    ...
    # Individual average losses different tasks 
    # =========================================
    ...
    if only_rrt:
        ...
        epoch_averages = average_global_loss_act, average_global_loss_ttne, average_global_loss_rrt
    ...
    # Outcome prediction loss
        ...
        epoch_averages = average_global_loss_act, average_global_loss_ttne, average_global_loss_out
    ...
    # Remaining Runtime prediction loss (MAE) and Outcome prediction loss
        ...
        epoch_averages = average_global_loss_act, average_global_loss_ttne, average_global_loss_rrt, average_global_loss_out
    
    elif both_not:
        epoch_averages = average_global_loss_act, average_global_loss_ttne


    # Logging average statistics current epoch to Tensorboard 

    # Logging average losses current epoch 
    if writer_bool: 
        log_variables(writer=writer, 
                        type_string='Loss', 
                        component_string_list=component_string_list, 
                        scalar_value_list=epoch_averages, 
                        training_iteration=epoch_number)

        # _______________
        # PCGrad statistics individual task gradients 
        # ===============
        
        # Average BatchWeight current epoch 
        # For definition batch_weight: 
        # see docstring `forward` method of `MultiOutputLoss_PCGrad` 
        # class defined in `train_utils_PCGrad.py`
        # ---------------
        log_variables(writer=writer, 
                        type_string='BatchWeight', 
                        component_string_list=component_string_list, 
                        scalar_value_list=average_batch_weight, 
                        training_iteration=epoch_number)
        

        # Logging average OG gradient norm (shared params) 
        # for the different tasks
        # ---------------
        log_variables(writer=writer, 
                        type_string='AverageOriginalGradientNorm', 
                        component_string_list=component_string_list, 
                        scalar_value_list=running_average_ognorm, 
                        training_iteration=epoch_number)
    
    # Logging average percentage of other tasks each task' gradient 
    # was conflicting with 
    # ---------------
    # Converting to numpy array
    np_running_task_conflict_percentage = running_task_conflict_fraction.cpu().numpy() # np.array of shape (num_tasks,)

    # converting to percentage 
    np_running_task_conflict_percentage = np_running_task_conflict_percentage * 100 

    if writer_bool: 
        log_variables(writer=writer, 
                        type_string='AveragePercentageOtherTasksConflicting', 
                        component_string_list=component_string_list, 
                        scalar_value_list=np_running_task_conflict_percentage, 
                        training_iteration=epoch_number)
    ...
    # _______________
    # PCGrad statistics relations OG gradients unique task pairs  
    # ===============

    # Converting numpy string list to native python list of strings 
    pairnames_string_list = [str(uni_pairname) for uni_pairname in unique_pairnames]

    # Logging average cosine similarity between gradients shared parameters 
    # of the unique task pairs (prior to gradient projection)
    # ---------------
    np_running_average_cosine_sims = running_average_cosine_sims.cpu().numpy() # np array of shape (num_unique_pairs,)

    if writer_bool: 
        log_variables(writer=writer, 
                        type_string='AverageCosineSimilarity', 
                        component_string_list=pairnames_string_list, 
                        scalar_value_list=np_running_average_cosine_sims, 
                        training_iteration=epoch_number)

    # Logging percentage of iterations current epoch in which gradients 
    # shared parameters of each unique task pairs (prior to gradient projection)
    # conflicted with each other (negative cosine similarity)
    # ---------------
    np_running_percentage_conflicting = running_percentage_conflicting.cpu().numpy() * 100 # np array of shape (num_unique_pairs,)

    if writer_bool:
        log_variables(writer=writer, 
                        type_string='PercentageIterationsConflicting', 
                        component_string_list=pairnames_string_list, 
                        scalar_value_list=np_running_percentage_conflicting, 
                        training_iteration=epoch_number)
...
def train_model(model, 
    ...
    (MTO) technique "PCGrad" by Yu et al. [1]_. 
    ...
    writer_bool : bool, optional
        Whether to log training/validation metrics and PCGrad diagnostics
        to TensorBoard. Defaults to ``False`` because the extra logging
    ...
    loss_fn = MultiOutputLoss_PCGrad(num_classes, 
                                 remaining_runtime_head, 
                                 outcome_bool, 
                                 clen_dis_ref, 
                                 shared_layers,
    ...
    decay_factor = 0.96
    optimizer = torch.optim.AdamW(model.parameters(), lr=lr_model, weight_decay=0.0001)
    ...
    for epoch in range(start_epoch, start_epoch + num_epochs):
        ...
        model, optimizer, epoch_averages = train_epoch(model, 
                                                       ...
                                                       writer, 
                                                       writer_bool)
        # Logging average training losses current epoch
        train_losses_act.append(epoch_averages[0])
        train_losses_ttne.append(epoch_averages[1])
        if only_rrt:
            train_losses_rrt.append(epoch_averages[2])
        elif only_out:
            train_losses_out.append(epoch_averages[2])
        elif both:
            train_losses_rrt.append(epoch_averages[2])
    ...
    epoch_list = [i for i in range(len(train_losses_act))]
