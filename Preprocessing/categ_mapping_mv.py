...
def missing_val_cat_mapping(train_df, 
    ...
    four lists, `cat_casefts`, `num_casefts`, `cat_eventfts` or 
    `num_eventfts`, or as `case_id`, `act_label`, `timestamp`, `outcome` 
    or `outcome_mask`) will be discarded. 
    ...
    * two dataframes, the resulting train_df and test_df, both with 
    ...
    outcome_mask: str or None
        If needed, name of the integer column containing for each case 
        the event index of the event revealing the binary outcome label. 
        Needed for creating a boolean outcome mask further down the 
    ...
    if outcome_mask: 
        needed_cols.append(outcome_mask)
