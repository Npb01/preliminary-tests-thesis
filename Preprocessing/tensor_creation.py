...
def generate_prefix_tensors(case_id, 
    ...
    # Initialize 0 tensor, 0 will serve here as the padding value for the numerical prefix features.
    ...
    for cat_col in cat_cols:
        ...
        # Update zero tensor. Zero is the padding token, so we shift all indices by 1.
...
def generate_suffix_tensors(case_id, 
    ...
    # Initialize -1 tensor 
    # -1 here will serve as the padding value for the numerical suffix features.
...
def generate_actLabel_tensors(case_id, 
    ...
    act_lab_tens = tf.Tensor
        ...
        is 'car'+1. This is due to the fact that here we have to account for 
        the additional END_TOKEN (idx 'car'). 
    """
    ...
    # Initializing activity label tensor by filling it with 0s. Serves as padding label. 
...
def generate_outLabel_tensor(case_id, 
    ...
    outcome : str, optional 
        String representing the name containing the outcome column in the
        `log` dataframe. By default `None`. The default should be retained
        if the event log does not contain an outcome label, or if no tensor 
        containing the outcome labels for each prefix-suffx pair 
        (aka instance) should be generated. 
        Note: if not `None` and if `outcome_type='binary_outcome'`, the 
        ...
        to the same case ID. If not `None` and if 
        `outcome_type='multiclass_outcome'`, the respective column should
        should also be of the integer dtype, containing the already integer 
        encoded class labels. The class labels should again be
        constant for every event (i.e. dataframe row) pertaining
        to the same case ID.
    outcome_type : {'binary_outcome', 'multiclass_outcome'}, optional
        Indicates whether the ``outcome`` column is binary (default) or
        multi-class. Ignored when ``outcome=None``. `None` by default.
    outcome_mask : str, optional 
        If an outcome column is requested and hence specified for 
        the `outcome` parameter, and if this outcome is derived from 
        specific event feature information, such as, e.g., the 
        outcome labels for the BPIC17 event logs being derived from the 
        last occurring 'O activity', the `outcome_mask` parameter should 
        be given the string representing the column name of the integer 
        column containing for each case the event index of the event 
        revealing the outcome label for that case. This is needed 
        to create an outcome mask, 
        masking those instances (i.e. prefix-suffix pairs) of which 
        the prefix (model input) already contains the event revealing the 
        outcome label and hence for preventing data leakage. 
        This parameter, if specified, is only taken into account in case 
        an outcome column is given to the `outcome` parameter. The 
        default of `None` should be retained if `outcome=None` or in case 
        the outcome is not derived from information revealed in 
        any of the events, and hence in case no outcome mask is needed. 
    str_to_int : dict 
        Integer mapping for string instance IDs. 

    """
    # Map instance IDs to integers consistent with the prefix, suffix, timestamp 
    # and remaining runtime labels for consisting indexing 
    outcomeLabel_df['prefix_id_int'] = outcomeLabel_df[case_id].map(str_to_int)

    idx = torch.from_numpy(outcomeLabel_df['prefix_id_int'].to_numpy()) # shape (num_prefs, )

    # # Derive tensor containing the outcome labels for all 'num_prefs' instances 
    # out_updates = torch.from_numpy(outcomeLabel_df[outcome].to_numpy()) # shape (num_prefs, )
    # # convert to needed dtype binary CE
    # out_updates = out_updates.to(torch.float32)

    # Derive total number of instances (also referred to as 'prefix-suffix pairs', 'prefixes', ...)
    num_prefs = max(outcomeLabel_df['prefix_id_int'])+1

    # # Initialize zero filled tensor of shape (num_prefs, 1)
    # outLabel_tens = torch.zeros(size=(num_prefs,1), dtype=torch.float32)
    # # Insert the binary outcome labels on indices consistent with all other tensors 
    # outLabel_tens[idx, 0] = out_updates # shape (num_prefs, 1)

    if outcome_type == 'multiclass_outcome':
        out_updates = torch.from_numpy(outcomeLabel_df[outcome].to_numpy()).to(torch.int64)
        outLabel_tens = torch.zeros(size=(num_prefs,), dtype=torch.int64)
        outLabel_tens[idx] = out_updates
    else:
        # Binary outcome (default)
        out_updates = torch.from_numpy(outcomeLabel_df[outcome].to_numpy()).to(torch.float32)
        outLabel_tens = torch.zeros(size=(num_prefs,1), dtype=torch.float32)
        # Insert the binary outcome labels on indices consistent with all other tensors 
        outLabel_tens[idx, 0] = out_updates # shape (num_prefs, 1)
    ...
    # outLabel_tens = outLabel_tens.unsqueeze(1).to(torch.float32) # (num_prefs, 1)

    if outcome_mask: 
        # Derive bool mask of shape (num_prefs,), evaluating to 
        # True if an instance (/ prefix-suffix pair / prefix) should be 
        # masked from contributing to the binary CE loss or to the 
        # outcome prediction evaluation metrics because of the outcome 
        # label being derived from event information already 
        # contained within the prefix events (that serve as inputs) 
        # to the multi-task model 

        # Create bool column in df 
        # This formula holds since the integer `'outcome_mask'` column contains 
        # 0-based event indices, while 'prefix_nr' is 1-based. 
        outcomeLabel_df['mask_needed'] = outcomeLabel_df['prefix_nr'] > outcomeLabel_df[outcome_mask] 

        # Derive bool tensor from created column
        out_mask_updates = torch.from_numpy(outcomeLabel_df['mask_needed'].to_numpy()) # shape (num_prefs,)

        # Initialize final bool outcome mask 
        instance_mask_outcome = torch.zeros(size=(num_prefs,)) # shape (num_prefs,)
        instance_mask_outcome = instance_mask_outcome.to(torch.bool) 

        # Insert bool values on indices consistent with all other tensors 
        instance_mask_outcome[idx] = out_mask_updates

        return outLabel_tens, instance_mask_outcome

    else:
        return (outLabel_tens,)
...
def generate_tensordata(pref_suff, 
    ...
    outcome : str
        String representing the name containing the outcome column in the
        `log` dataframe. The default should be retained
        if the event log does not contain an outcome label, or if no tensor 
        containing the outcome labels for each prefix-suffx pair 
        (aka instance) should be generated. 
        Note: if not `None` and if `outcome_type='binary_outcome'`, the 
        ...
        constant for every event (i.e. dataframe row) pertaining
        to the same case ID.
    outcome_type : {'binary_outcome', 'multiclass_outcome'}
        Indicates whether the ``outcome`` column is binary (default) or
        multi-class. Ignored when ``outcome=None``.
    outcome_mask : str
        If an outcome column is requested and hence specified for 
        the `outcome` parameter, and if this outcome is derived from 
        specific event feature information, such as, e.g., the 
    ...
    if outcome: 
        outLabel_tensors = generate_outLabel_tensor(case_id, 
                                                    # act_label, 
                                                    outcomeLabel_df, 
                                                    outcome, 
                                                    outcome_type,
                                                    outcome_mask, 
                                                    str_to_int=str_to_int)
        # retrieving tensor containing outcome labels for all instances 
        outLabel_tens = outLabel_tensors[0] # (num_prefs, 1)

        all_tensors += (outLabel_tens, )

        if outcome_mask:
            # retrieving boolean tensor containing True for those instances of 
            # which one of the prefix events already reveals the outcome 
            instance_mask_outcome = outLabel_tensors[1] # (num_prefs,)
    ...
    if outcome and outcome_mask:
        return all_tensors, og_caseint_tensor, instance_mask_outcome
    else:
...
def generate_tensordata_train_test(train_pref_suff, 
    ...
    # Train set tensors: 
    print("Computing train set tensors")
    train_tensors = generate_tensordata(pref_suff = train_pref_suff, case_id = case_id, 
                                        act_label = act_label, cardinality_dict = cardinality_dict, 
                                        num_cols_dict = num_cols_dict, 
                                        cat_cols_dict = cat_cols_dict, 
                                        window_size = window_size, 
    ...
    print("____________________________")
    print("Computing validation set tensors")
    val_tensors = generate_tensordata(pref_suff = val_pref_suff, case_id = case_id, 
                                      act_label = act_label, cardinality_dict = cardinality_dict, 
                                      num_cols_dict = num_cols_dict, 
                                      cat_cols_dict = cat_cols_dict, 
                                      window_size = window_size, 
    ...
    print("____________________________")
    print("Computing test set tensors")
    test_tensors = generate_tensordata(pref_suff=test_pref_suff, case_id = case_id, 
                                       act_label = act_label, cardinality_dict = cardinality_dict, 
                                       num_cols_dict = num_cols_dict, 
                                       cat_cols_dict = cat_cols_dict, 
                                       window_size = window_size, 
    ...
    # test_data, og_caseint_test

    # if outcome and outcome_mask: 
    #     instance_mask_out_train = train_tensors[-1]
    #     instance_mask_out_val = val_tensors[-1]
    #     instance_mask_out_test = test_tensors[-1]
